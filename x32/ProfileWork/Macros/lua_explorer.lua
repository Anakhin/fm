...
local function makeItem(key, sval, vt)
	return {
		text	= ('%-25s %-9s %-20s'):format(valfmt(key), vt, sval),
		key 	= key
	}
end
...
local function makeMenuItems(obj)
	...
	if obj.properties and not rawget(obj, 'properties') then
	...
	-- do not create an empty menu
	if #items == 0 then
		table.insert(items, {text = 'empty', disable = true})
	end
...
function editValue(obj, key, title, del)
	if del then
...
function insertValue(obj, title)
...
-- we start here with the global context
process(_G, '_G')
