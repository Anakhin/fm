local function SelectBlock(info, sel, pos, blockType)
  if not sel then
    ...
    if info.CurLine < sel.EndLine or (info.CurLine == sel.EndLine and info.CurPos < sel.EndPos) then
      ...
      if info.CurLine == sel.StartLine and sel.StartPos == pos then
        editor.Select(info.EditorId, 0)
      else
...
local function SmartHome(select, blockType)
  ...
  if info then
    local str = editor.GetString(-1, line)
    ...
    for i = 1, len do
      ...
      if c ~= " " and c ~= "\t" then
        ...
        editor.SetPosition(info.EditorId, line, pos)
        if (select) then
    ...
    if not set then
      editor.SetPosition(info.EditorId, line, 1)
    end
...
local function SmartEnd(select, blockType)
  ...
  if info then
    ...
    if not set then
      editor.SetPosition(info.EditorId, line, len + 1)
    end
 end
end
